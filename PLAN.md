...
- **Update (2026-03-07 round-trip orientation)**: Fixed model→text→model side-clip roll drift for red/orange connector edge ports by switching rod-side orientation alignment from a hardcoded connector Y-axis heuristic to a port-tangent basis (`cross(connector_normal, connector_port_direction)`) in both preview snapping and topology solve paths (`frontend/src/helpers/snapHelper.ts`, `frontend/src/services/topologySolver.ts`). Preserved `fixed_roll` through topology resolution and compact text emission (`@ 0!`), and added regressions in `topologySolverRoundTrip.test.ts` + `topologyCompactFormat.test.ts`.
